...
CountryCode = pd.read_csv("data/CountryCode.csv")
PISAdata = PISAdata.merge(CountryCode, on="COUNTRY_CODE", how="left")
PISAdata['GRADE'] = PISAdata['GRADE'].astype(str)

country = st.selectbox("Country", PISAdata['COUNTRY'].unique().tolist())

filtered_country = PISAdata.query(f"`COUNTRY` == '{country}'")
...
with fig_col1:
    ...
    fig1 = px.pie(
        ...
        labels={'GENDER':'Percentage of bullied', 'bullied':'bully'}, width=400
    )
...
with fig_col2:
    ...
    fig2 = px.bar(df2, x="GRADE", y="Percentage of bullied", title='Percentage of Bullied by Grade',
                width=600, hover_data=['Percentage of bullied'], labels={'GRADE':'Grade'})
...
fig3 = px.choropleth(df3, locations="COUNTRY_CODE", title='Percentage of Bullied by Country',
                    color="Percentage of bullied", # lifeExp is a column of gapminder
                    hover_name="COUNTRY_CODE", # column to add to hover information
                    color_continuous_scale=px.colors.sequential.Blues, width=1100)
